...
export function CollectionsTable({ onEdit }: CollectionsTableProps) {
          ...
          cell: ({ row }) => {
            ...
            return isFeatured ? (
              <Badge className="border-yellow-200 bg-yellow-100 text-yellow-700 hover:bg-yellow-100">
                <StarIcon className="mr-1 size-3.5 fill-yellow-500 text-yellow-500" />
                Featured
              </Badge>
            ) : (
   ...
   */
  if (isLoading) {
    return <CollectionsTableSkeleton />;
  }
  ...
  if (!pagination?.total && !debouncedSearch) {
    return (
      <EmptyState
        ...
        description="Create your first collection to start organizing your products."
        action={
          <Button>
            <FolderIcon />
            Create Collection
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="featured-asc">Featured last</SelectItem>
          </SelectContent>
        </Select>
      </div>
